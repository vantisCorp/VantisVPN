...
fn bench_key_generation(c: &mut Criterion) {
    c.bench_function("key_generation", |b| {
        b.iter(|| black_box(EphemeralKeyPair::new().unwrap()))
    });
}

fn bench_encryption(c: &mut Criterion) {
    let key_pair = EphemeralKeyPair::new().unwrap();
    let mut encrypt_ctx = EncryptionContext::new(
    ...
    c.bench_function("encryption_1kb", |b| {
        b.iter(|| black_box(encrypt_ctx.encrypt_packet(black_box(&data)).unwrap()))
    });
}
...
fn bench_decryption(c: &mut Criterion) {
    ...
    .unwrap();
    let mut decrypt_ctx = DecryptionContext::new(
        key_pair.public_key().as_bytes(),
        CipherSuite::ChaCha20Poly1305,
    )
    .unwrap();
    ...
    c.bench_function("decryption_1kb", |b| {
        b.iter(|| black_box(decrypt_ctx.decrypt_packet(black_box(&encrypted)).unwrap()))
    });
}
...
fn bench_hash_computation(c: &mut Criterion) {
    ...
    c.bench_function("hash_1kb", |b| {
        b.iter(|| black_box(hash_instance.compute(black_box(&data)).unwrap()))
    });
}
