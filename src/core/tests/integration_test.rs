...
fn test_full_encryption_flow() {
    ...
    let mut encrypt_ctx = EncryptionContext::new(public_key.as_bytes(), CipherSuite::ChaCha20Poly1305).unwrap();
    let mut decrypt_ctx = DecryptionContext::new(public_key.as_bytes(), CipherSuite::ChaCha20Poly1305).unwrap();
