...
impl ZeroTrust {
    ...
    pub async fn update_policy(&mut self, policy: ZeroTrustPolicy) -> Result<(), VantisError> {
        ...
        if !policies.contains_key(&policy.id) {
            return Err(VantisError::NotFound(format!(
                "Policy not found: {}",
                policy.id
            )));
        }
    ...
    pub async fn evaluate_access(
    ...
    ) -> Result<AccessDecision, VantisError> {
        ...
        let policies = self.policies.lock().await;
        let mut matching_policies: Vec<ZeroTrustPolicy> = policies
            .values()
        ...
        for policy in matching_policies {
            match policy.action {
                PolicyAction::Allow => {
                    self.log_access(&request, &policy, true, "Allowed by policy")
                        .await;
                    return Ok(AccessDecision {
                        allowed: true,
                ...
                PolicyAction::Deny => {
                    ...
                    self.log_access(&request, &policy, false, "Denied by policy")
                        .await;
                    return Ok(AccessDecision {
                        allowed: false,
                        policy_id: Some(policy.id.clone()),
                ...
                PolicyAction::RequireAuth => {
                    if request.user_id.is_some() {
                        self.log_access(&request, &policy, true, "Authenticated access allowed")
                            .await;
                        return Ok(AccessDecision {
                            allowed: true,
                            policy_id: Some(policy.id.clone()),
                        ...
                        self.log_access(&request, &policy, false, "Authentication required")
                            .await;
                        return Ok(AccessDecision {
                            allowed: false,
                            policy_id: Some(policy.id.clone()),
                ...
                PolicyAction::RequireMfa => {
                    self.log_access(&request, &policy, false, "MFA required")
                        .await;
                    return Ok(AccessDecision {
                        allowed: false,
                        policy_id: Some(policy.id.clone()),
                ...
                PolicyAction::LogOnly => {
                    self.log_access(&request, &policy, true, "Logged only")
                        .await;
                    continue;
                },
            }
        ...
        match self.config.default_action {
            PolicyAction::Allow => {
                self.log_access(&request, &ZeroTrustPolicy::default(), true, "Default allow")
                    .await;
                Ok(AccessDecision {
                    allowed: true,
                    policy_id: None,
            ...
            _ => {
                self.record_failed_attempt(&request).await;
                self.log_access(&request, &ZeroTrustPolicy::default(), false, "Default deny")
                    .await;
                Ok(AccessDecision {
                    allowed: false,
                    policy_id: None,
    ...
    fn policy_matches(&self, policy: &ZeroTrustPolicy, request: &AccessRequest) -> bool {
        ...
        if policy.protocol != "any"
            && policy.protocol.to_lowercase() != request.protocol.to_lowercase()
        {
            return false;
        }
        ...
        if let Some(user_id) = &request.user_id {
            ...
            *count += 1;
            ...
            if *count >= self.config.max_failed_attempts {
                ...
                lockout.insert(
                    user_id.clone(),
                    Utc::now() + Duration::seconds(self.config.lockout_duration as i64),
                );
            }
        }
    }
...
mod tests {
    ...
    async fn test_device_trust() {
        ...
        zt.update_device_trust("device1".to_string(), 85, vec!["updated".to_string()])
            .await;
