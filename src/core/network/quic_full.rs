    ...
    pub fn deserialize(data: &[u8]) -> Result<Self> {
        ...
        if offset + dcid_len > data.len() {
            return Err(VantisError::InvalidPacket(
                "Invalid destination CID length".into(),
            ));
        }
        ...
        if offset + scid_len > data.len() {
            return Err(VantisError::InvalidPacket(
                "Invalid source CID length".into(),
            ));
        }
        ...
        let packet_number = match pn_len {
            0 => {
                if offset >= data.len() {
                    return Err(VantisError::InvalidPacket(
                        "Missing packet number byte".into(),
                    ));
                }
                data[offset] as u64
            },
            1 => {
                if offset + 2 > data.len() {
                    return Err(VantisError::InvalidPacket(
                        "Missing packet number bytes".into(),
                    ));
                }
            ...
            _ => {
                return Err(VantisError::InvalidPacket(
                    "Invalid packet number length".into(),
                ))
            },
        };
...
impl QuicEndpoint {
    ...
    async fn handle_packet(&self, data: Vec<u8>, _src: SocketAddr) -> Result<()> {
        ...
        debug!(
            "Received QUIC packet: type={:?}, pn={}",
            header.packet_type, header.packet_number
        );
