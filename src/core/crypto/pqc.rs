...
impl HybridKeyExchange {
    ...
    pub fn derive_shared_secret(
        ...
        let classical_pair = self.classical_key.as_ref()
            .ok_or_else(|| crate::VantisError::KeyConsumed)?;
        let pqc_pair = self.pqc_key.as_ref()
            .ok_or_else(|| crate::VantisError::KeyConsumed)?;
        
        // Derive classical secret (X25519-like)
        let classical_public_key = super::keys::PublicKey::new(
            classical_public.try_into().map_err(|_| crate::VantisError::InvalidKeySize)?
        );
...
mod tests {
    ...
    fn test_dilithium_sign_verify() {
        ...
        let signature = DilithiumSignature::sign(message, &secret)
            .expect("Failed to sign");
        
        let verified = signature.verify(message, &public)
    ...
    fn test_hybrid_key_exchange() {
        ...
        let (pqc_ciphertext, _shared_alice) = KyberKEM::encapsulate(&bob_pqc)
            .expect("Encapsulation failed");
        
        // Bob derives shared secret
        let shared_bob = bob.derive_shared_secret(&alice_classical, &pqc_ciphertext)
